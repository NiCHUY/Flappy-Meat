...
QString score("BETA"), diff, bird_custom(""), custom_write("CUSTOM"), presset_custom(":/graphics/presset.jpg");
...
MainWindow::MainWindow(QWidget *parent)
    ...
    , birdicon(":/graphics/FlappyMeat.png")
{
    setWindowIcon(birdicon);
...
//buttons
    ...
    Easy_button->setGeometry(QRect(QPoint((x - button_w)/2, 400), QSize(button_w, button_h)));
    ...
    Medium_button->setGeometry(QRect(QPoint((x - button_w)/2, 465), QSize(button_w, button_h)));
    ...
    Hard_button->setGeometry(QRect(QPoint((x - button_w)/2, 530), QSize(button_w, button_h)));
    ...
    custom_button = new QPushButton(custom_write, this);
    custom_button->setGeometry(QRect(QPoint(x/2-200, 320), QSize(300, 40)));
    custom_button->setFocusPolicy(Qt::NoFocus);
    connect(custom_button, SIGNAL (released()), this, SLOT (birdcustom()));
    ...
    connect(custom_button_picker, SIGNAL (released()), this, SLOT (choosebird()));


    backgroundorigin_button = new QPushButton("Original Background", this);
    backgroundorigin_button->setGeometry(QRect(QPoint(x/2-200, 400), QSize(400, 40)));
    backgroundorigin_button->setFocusPolicy(Qt::NoFocus);
    connect(backgroundorigin_button, SIGNAL (released()), this, SLOT (background_origin()));

    custom_background_button = new QPushButton(custom_write, this);
    custom_background_button->setGeometry(QRect(QPoint(x/2-200, 440), QSize(300, 40)));
    custom_background_button->setFocusPolicy(Qt::NoFocus);
    connect(custom_background_button, SIGNAL (released()), this, SLOT (background_custom()));

    custom_background_button_picker = new QPushButton("...", this);
    custom_background_button_picker->setGeometry(QRect(QPoint(x/2+100, 440), QSize(100, 40)));
    custom_background_button_picker->setFocusPolicy(Qt::NoFocus);
    connect(custom_background_button_picker, SIGNAL (released()), this, SLOT (background_custom_picker()));
...
void MainWindow::randomanimation(){
    ...
    if(birdtype == 4 && bird_custom != ""){
                custom_write = bird_custom;
        bird = new QPixmap(bird_custom);
    }else{
        custom_write = "ERROR";
...
void MainWindow::paintEvent(QPaintEvent* paint_event) {
    ...
    label->setFont(font);
//background
    painter.drawPixmap(0, 0, this->width(), this->height(), *presset);
...
//score
    ...
    void MainWindow::ButtonChecker(){
        ...
        if(skins == true){
            ...
            backgroundorigin_button->setVisible(true);
            custom_background_button->setVisible(true);
            custom_background_button_picker->setVisible(true);
            ...
            backgroundorigin_button->setVisible(false);
            custom_background_button->setVisible(false);
            custom_background_button_picker->setVisible(false);
    ...
    void MainWindow::DifficultyChecker(){
        if(settings == true){
            ...
            label->setGeometry(this->width()/2-500,320,1000,80);
    ...
    void MainWindow::birdcustom(){
        birdtype = 4;
    }

    void MainWindow::choosebird(){

        bird_custom = QFileDialog::getOpenFileName(this, tr("Open File"), "/home", tr("Images (*.png)"));
    }

    void MainWindow::background_origin(){
        presset = new QPixmap(":/graphics/presset.jpg");
    }
    void MainWindow::background_custom(){
         presset = new QPixmap(presset_custom);
    }
    void MainWindow::background_custom_picker(){
         presset_custom = QFileDialog::getOpenFileName(this, tr("Open File"), "/home", tr("Images (*.png)"));
    }
